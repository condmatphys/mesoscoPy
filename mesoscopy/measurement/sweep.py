...
def sweep1d_repeat(
    ...
    *param_meas: doNd.ParamMeasT,
    ...
    meas = Measurement(exp=exp)
    if measure_retrace:
        meas_retrace = Measurement(exp=exp)
    ...
    with doNd._catch_keyboard_interrupts() as interrupted, \
        ...
        for i in range(num_repeat):
            ...
            if y % 2 == 0:
                ...
                for i in np.linspace(xarray[-1],xarray[0],num_retrace):
                    param_set.set(i)
                skip=True
...
def sweep2d(
    ...
    *param_meas: doNd.ParamMeasT,
    ...
    with doNd._catch_keyboard_interrupts() as interrupted, \
        ...
        print(f'sweeps: {datasaver.run_id}, retrace: {dataretrace.run_id}')
        ...
        for c, set_pointy in enumerate(yarray):
            ...
            if not skip:
                ...
                for set_point in tqdm(xisetpoints):
                    param_set.set(set_point)
                    time.sleep(inner_delay)
